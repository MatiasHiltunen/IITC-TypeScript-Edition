...
export const dialog = function(options) {
  ...
  let id = 'dialog-' + (options.modal ? 'modal' : (options.id ? options.id : 'anon-' + DIALOG_ID++));
  let jqID = '#' + id;
  let html = '';
  ...
  function sizeFix() {
    ...
    let options = dialog.dialog('option');
  ...
  // @ts-ignore
  let dialog = $(jqID).dialog($.extend(true, {
    autoOpen: false,
    modal: false,
    draggable: true,
    closeText: '',
    ...
    open: function() {
      let titlebar = $(this).closest('.ui-dialog').find('.ui-dialog-titlebar');
      titlebar.find('.ui-dialog-title')
        .addClass('ui-dialog-title-active')
        .addClass('text-overflow-ellipsis');
      let close = titlebar.find('.ui-dialog-titlebar-close');
      ...
      if(!(<any>$(this)).dialog('option', 'modal')) {
        // Start out with a cloned version of the close button
        let collapse = close.clone();
        ...
        collapse.click($.bind(function() {
          let collapsed = ($(this).data('collapsed') === true);
          ...
          if($(this).data('collapseExpandCallback')) {
            ...
            $.bind($(this).data('collapseExpandCallback'), this)(!collapsed);
          } else {
            if(!collapsed && $(this).data('collapseCallback')) {
              $.bind($(this).data('collapseCallback'), this)();
            } else if (collapsed && $(this).data('expandCallback')) {
              $.bind($(this).data('expandCallback'), this)();
            }
          }
          ...
          let content = dialog.find('.ui-dialog-content');
          let buttonpane = dialog.find('.ui-dialog-buttonpane');
          let button   = dialog.find('.ui-dialog-titlebar-button-collapse');
    ...
    /*   console.log('window.dialog: ' + $(this).data('id') + ' (' + $(this).dialog('option', 'title') + ') opened. ' + DIALOG_COUNT + ' remain.'); */
    },
    close: function() {
      // Run the close callback if we have one
      if($(this).data('closeCallback')) {
        $.bind($(this).data('closeCallback'), this)();
      }
      ...
      DIALOG_COUNT--;
      // @ts-ignore
      // console.log('window.dialog: ' + $(this).data('id') + ' (' + $(this).dialog('option', 'title') + ') closed. ' + DIALOG_COUNT + ' remain.');
    ...
    focus: function() {
      if($(this).data('focusCallback')) {
        $.bind($(this).data('focusCallback'), this)();
      }
      ...
      if(DIALOG_FOCUS && $(DIALOG_FOCUS).data('id') !== $(this).data('id')) {
        ...
        $.bind(function(event, ui) {
          if($(this).data('blurCallback')) {
            $.bind($(this).data('blurCallback'), this)();
          }
      ...
      if (typeof android !== 'undefined' && android && android.dialogFocused) {
        // @ts-ignore
        android.dialogFocused($(DIALOG_FOCUS).data('id'));
      }
 ...
 */
export const alert = function(text:any, isHTML, closeCallback) {
  let obj: {html?:string, text?:string, closeCallback:any} = {closeCallback: closeCallback};
  if(isHTML) {
    obj.html = text;
  } else {
    obj.text = text;
